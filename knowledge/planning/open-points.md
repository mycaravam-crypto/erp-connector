...
| 6 | Return-channel timing | Vendor + sponsor | Iteration 2 scope. The vendor will eventually write back CI update confirmations; timing and exact field list still TBD (pending the vendor ICD contract). Design fully resolved in [Import Definitions](/pipeline/import-definitions.md) §6 — root-match key (the existing `Guid`), writable scope (confirmation/status fields on the root only), partial-file/GDPR/gap-detection policy are all decided; implementation not started. What's genuinely still pending here is *when* the vendor can deliver a return channel at all and the concrete column names, not the design. |
