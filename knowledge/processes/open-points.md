...
| 5 | Snapshot volume | ERP data steward | If the CI count exceeds ~500 k: add a pagination or delta parameter to `IErpReader.ReadMaintainableCIsAsync`. Currently a full-snapshot read with no server-side paging. |
...
| 8 | Allocation chart import | ERP + vendor | Defines the scope predicate for the production `IErpReader`. The demo reader uses `MaintenancePlanStatus = Active`; the production predicate may also depend on allocation chart references. |
...
- [IErpReader](/pipeline/erp-reader.md) — #5 and #8 affect the ERP reader interface and implementation
