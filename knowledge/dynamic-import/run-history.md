...
tags: [domain, infrastructure, dynamic-import]
...
Definitions §4](/pipeline/import-definitions.md#4-inbound-flow)).
...
1. **`ImportNodeWalker.WalkAsync`** parses the inbound `ImportEnvelope`, resolves each record's
   root-row match, and produces an `ImportWalkResult` — a per-row field-level diff
...
2. **`ImportPlanBuilder.Build`** reshapes that diff into the persisted `ImportPlan`: root-row field
   diffs become `ImportPlanOperation`s (child-table diffs stay preview-only — no real
   `ImportDefinition` writes to a child table yet, see `ImportNode`'s `OnMissingChild` note); an
   unchanged column produces no operation, since there's nothing to write.
...
2. **Zero affected rows** means the ERP row's state moved on since the plan was built; that row is
   counted in `ConflictCount`, excluded, and left untouched — it does **not** fail the run. One
   stale or bad row doesn't block every good one in the same file.
...
the list view's data source. `GET /api/import-runs/{id}` returns one run's full detail *including*
`PlanJson`'s operations — the review dialog needs the actual per-row diff before an Approver can
meaningfully decide, which the summary endpoint alone doesn't carry. Unlike release/reject, this is
a plain read available regardless of `Status` — revisiting a `Released`/`Rejected`/`Failed` run's
plan after the fact is useful too.
...
- [Import Definitions §4](/pipeline/import-definitions.md#4-inbound-flow) — the full flow, steps 5-8
