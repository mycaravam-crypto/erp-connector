...
tags: [pipeline, orchestration, background-service, dynamic-import]
timestamp: 2026-09-06T00:00:00Z
---

`ImportWorker` is the automatic half of the inbound pipeline — polling `inbound/` so a saved
`ImportDefinition` doesn't need a human watching a folder. The manual half,
`POST /api/import-definitions/{id}/runs`, runs the same walk/plan/stage steps (3, 5, 6 below)
...
Definitions §3](/pipeline/import-definitions.md#3-comparison-with-the-export-side). It is a **sibling** of
...
3. **Parse + route.** The file itself is parsed as JSON; its top-level `definition` property (the
   ...
   same way as a manifest failure.
4. **Idempotency check.** `(ImportDefinitionId, Sha256Checksum)` is looked up against existing runs
   *before* parsing further. A match is classified via `ImportWorker.ClassifyDuplicate` —
   already-staged (`PendingReview`), already-released (`Released`), rejected duplicate
   (`Rejected`), or a bare "duplicate" for anything else (`Failed`) — audit-logged
   (`import_duplicate_detected`), and the file is moved to `inbound/processed/` without creating a
   second `ImportRunEntity`. A race between two pollers landing on the same pair is caught by the
   DB's own unique constraint on `INSERT` and handled identically, rather than crashing the poll.
5. **Walk + plan.** `ImportNodeWalker.WalkAsync` parses the envelope against the definition's
   `RootNode`, then `ImportPlanBuilder.Build` reshapes that into the persisted `ImportPlan`. An
   `ImportValidationException` from the walker (a structural problem with the saved definition, not
...
approved run is [`ImportRunReleaser`](run-history.md)'s job, triggered by a human via
...
- [Import Definitions §4](/pipeline/import-definitions.md#4-inbound-flow) — the full inbound flow, steps 1-5
