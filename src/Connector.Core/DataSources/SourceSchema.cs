...
/// <c>information_schema.columns</c>' own <c>is_identity</c>/<c>is_generated</c> columns. Added for Phase 17
/// Slice 5 (import-definitions.md §6 Open Decision #9): the save-time AllowedWritableColumns validator needs
/// to reject a TargetColumn the ERP itself manages (an identity sequence or a GENERATED ... STORED
/// expression), not just one that's the primary key or an FK. Both default false so every existing
/// <see cref="SourceColumn"/> call site (the hardcoded demo schema) stays valid without updating.
/// </remarks>
