...
<template>
  <div class="flex flex-col gap-2">
    <div
      ...
      :key="idx"
      :class="['flex gap-3 items-start px-4 py-3 border rounded-lg bg-surface', node.enabled ? 'border-brand/25 bg-brand/10' : 'border-border opacity-70']"
    ...
    >
      ...
      <div class="flex-1 flex flex-col gap-2">
        <div class="flex gap-2.5 flex-wrap items-end">
          <div class="flex flex-col gap-1 min-w-32">
            <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Export key</label>
            <input
              type="text"
          ...
          <span class="text-xs bg-surface-elevated text-text-secondary px-1.5 py-1 rounded-full self-end mb-1">{{ node.kind }}</span>

          <template v-if="node.kind === 'scalar-field'">
            <div class="flex flex-col gap-1 min-w-40">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Source column ({{ contextTable }})</label>
              <select
          ...
          <template v-else>
            <div class="flex flex-col gap-1 min-w-36">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Related table</label>
              <select
            ...
            <div class="flex flex-col gap-1 min-w-32">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Join column (in {{ node.relatedTable || '…' }})</label>
              <select
                v-model="node.joinKey"
                :disabled="!node.relatedTable"
                class="px-2 py-1 border border-border-strong rounded text-sm text-text-primary bg-surface disabled:bg-surface-elevated"
                @change="emit('dirty')"
              >
                <option value="" disabled>— select —</option>
            ...
            <div class="flex flex-col gap-1 min-w-32">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Matches column (in {{ contextTable }})</label>
              <select
            ...
            <div class="flex flex-col gap-1 w-40">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Shape</label>
              <select
                v-model="node.kind"
                class="px-2 py-1 border border-border-strong rounded text-sm text-text-primary bg-surface"
                @change="emit('dirty')"
            ...
            <div class="flex flex-col gap-1 min-w-40 flex-1">
              <label class="text-[0.65rem] font-semibold text-text-muted uppercase tracking-wide">Filter (optional)</label>
              <input
                type="text"
                v-model="node.filter"
                placeholder="e.g. is_active = true"
                class="px-2 py-1 border border-border-strong rounded text-sm text-text-primary font-mono outline-none focus:border-brand"
                @input="emit('dirty')"
              />
            </div>
          </template>
        ...
        <div v-if="node.kind === 'object' || node.kind === 'array'" class="mt-1 pl-4 border-l-2 border-border">
      ...
      <button
        type="button"
        class="shrink-0 p-1 border border-danger rounded text-danger bg-surface leading-none cursor-pointer hover:bg-danger-bg"
        title="Remove"
    ...
    <div v-if="depth < MAX_NESTED_DEPTH" class="relative">
      ...
      <div v-if="showAddMenu" class="mt-1 flex gap-2">
        <button
          type="button"
          class="px-2.5 py-1 border border-border rounded text-xs text-text-secondary bg-surface-elevated cursor-pointer hover:bg-surface-elevated"
