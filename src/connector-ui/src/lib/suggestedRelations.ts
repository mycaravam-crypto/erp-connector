 ...
 * actually express:
 *  - forward: a column on `tableName` itself pointing at another table's key (e.g.
 *    `item.manufacturer_id -> manufacturer.id`) is a 1:1 lookup, suggested as an "object".
 *  - reverse: another table's column pointing back at `tableName` (e.g.
 *    `address.item_id -> item.id`) is a 1:N collection, suggested as an "array".
 * Shared by the legacy mapping tree (SchemaView.vue, which only wants the reverse/array shape —
 * see its `.filter(s => s.kind === 'array')`) and the Export Definitions tree builder
...
export function findSuggestedRelations(
  ...
  const thisTable = schema.tables.find((t) => t.name === tableName)
  for (const c of thisTable?.columns ?? []) {
    if (c.foreignKeyTable && c.foreignKeyTable !== tableName && c.foreignKeyColumn) {
      suggestions.push({
        relatedTable: c.foreignKeyTable,
        joinKey: c.foreignKeyColumn,
        sourceJoinKey: c.name,
        kind: 'object',
      })
    }
  }
