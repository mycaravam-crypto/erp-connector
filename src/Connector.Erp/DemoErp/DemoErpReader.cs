...
/// </remarks>
public sealed class DemoErpReader(DemoErpDbContext db, ILogger<DemoErpReader> logger) : IErpReader
{
    public async Task<IReadOnlyList<ErpConfigurationItem>> ReadMaintainableCIsAsync(CancellationToken ct)
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            throw new ErpConnectionException(
                $"Demo-ERP nicht erreichbar oder Abfrage fehlgeschlagen: {ex.Message}",
                ex
            );
        }
    }
